 ...
 * @参数说明：mux         :  复用 GPIO 引脚
 * @函数返回：无
 * @调用方法：GtimPwm MyPwm(88, 2, LS_GTIM_INVERSED, 50, 2000);
    ...
    : Gpio(gpio), Pol(pol), Period(period), Duty_cycle(duty_cycle), ChNum(chNum - 1)
{
    ...
    // 启动计数器和预装载值
    LS_writel(AddressMap(LS_GTIM_BASE_ADDR + LS_GTIM_CR1, PAGE_SIZE), 0b10000001);
}
 ...
 * @备注说明：假设要设置 50Hz 周期，那该参数直接填 50 即可，单位为 Hz
 QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ*/
void GtimPwm::SetPeriod(uint32_t cycle)
{
    this->Disable();
    this->Period = cycle;
    uint32_t val = LS_GTIM_CLK / this->Period - 1;
    LS_writel(this->GTIM_ARR, val);
    this->Enable();
}
 ...
 * @备注说明：无论周期设置为多少，占空比的范围都是 0~10000
 QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ*/
void GtimPwm::SetDutyCycle(uint32_t duty_cycle)
{
    pthread_mutex_lock(&Gtim_mutex);
    this->Duty_cycle = duty_cycle;
    uint32_t val = LS_GTIM_CLK / this->Period * this->Duty_cycle / LS_GTIM_DUTY_MAX;
    LS_writel(this->GTIM_CCRx, val);
