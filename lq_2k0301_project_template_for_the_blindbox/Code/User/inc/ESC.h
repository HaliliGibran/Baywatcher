...
public:
    ...
    // ================== 动态负压相关配置 ==================
    bool enable_dynamic_esc;       // 是否开启动态负压
    float dynamic_esc_base;        // 动态负压基础值 (默认百分比)
    float dynamic_esc_max;         // 动态负压最大上限百分比
    float dynamic_esc_threshold;   // factor 死区阈值，低于此时维持基础值
    float dynamic_esc_factor_max;  // factor 最大参考值，高于此时维持最大百分比
    float current_factor;          // 由外部（PID控制环）实时传入的当前 factor
    float lp_alpha;                // 一阶低通滤波系数 (0.0~1.0，越小越平滑)
    float filtered_target_base;    // 低通滤波后的目标推力

private:
    bool is_initialized = false;
    uint32_t esc_log_cnt = 0;      // 日志节拍计数器
