...
用于在该行上检测白红带参考包络的 x 范围。

在 `y = 320` 参考行上检测白红带参考包络，得到：

reference_x_range = [reference_x_min, reference_x_max]
...
白红带参考包络 与 搜索带内严格红带 x 范围 的最大包络

即：
x0 = min(reference_x_min, red_x_min)
x1 = max(reference_x_max, red_x_max)
...
最终执行摘要
1. 在 `y=120..320` 内建立基础搜索框
2. 用白红带参考包络与严格红带最大包络确定最终搜索框 `x`
