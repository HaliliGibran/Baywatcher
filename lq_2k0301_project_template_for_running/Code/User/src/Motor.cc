...
// ================= 阶梯加速 =================
BayWatcher_Speed_Ramper::BayWatcher_Speed_Ramper() {
    current_ramped_speed = 0.0f;
}

void BayWatcher_Speed_Ramper::Reset() {
    current_ramped_speed = 0.0f;
}

float BayWatcher_Speed_Ramper::Calc_Ramped_Speed(float target_speed, float step) {
    // 阶梯加速
    if (current_ramped_speed < target_speed) {
        current_ramped_speed += step;
        // 如果加过头了，强制等于目标速度 (阶梯过程结束)
        if (current_ramped_speed > target_speed) {
            current_ramped_speed = target_speed;
        }
    } 
    // 阶梯减速 (向下兼容)
    else if (current_ramped_speed > target_speed) {
        current_ramped_speed -= step;
        if (current_ramped_speed < target_speed) {
            current_ramped_speed = target_speed;
        }
