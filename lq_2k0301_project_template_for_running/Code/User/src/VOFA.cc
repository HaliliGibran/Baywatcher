...
switch (current_mode) {
    ...
    if (n > 0) {
        ...
        // else if (strncmp(buf, "start0", 6) == 0) {
        //     // 上位机发 start0，测试带负压起步
        //     BayWatcher_Start_Car(0, 100.0f, 1000, 0.5f);
        // }
        // else if (strncmp(buf, "start1", 6) == 0) {
        //     // 上位机发 start1，测试不带负压起步（比如车放在桌子上空转调速）
        //     BayWatcher_Start_Car(1, 100.0f, 0, 0.5f); // 模式1时，延时填0就行
        // }
        // 外环模糊 PID 调参
        // else if (strncmp(buf, "bkp:", 4) == 0) KP_Base = atof(buf + 4);
        // else if (strncmp(buf, "bkd:", 4) == 0) KD_Base = atof(buf + 4);
