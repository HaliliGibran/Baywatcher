...
void BayWatcher_VOFA::Send_Data(float target_L, float speed_L, int pwm_L, 
                                ...
                                float preview_curve_angle_deg, float preview_img_y,float big_langd_add) {
...
//                                 float yaw, float current_yawspeed) {
    ...
    int len = sprintf(buf, "%.2f,%.2f,%d,%.2f,%.2f,%d,%.2f,%.2f,%.4f,%.2f,%.2f\n", 
                      target_L, speed_L, pwm_L, target_R, speed_R, pwm_R,
                      yaw, base_speed, preview_curve_angle_deg, preview_img_y,big_langd_add);
...
switch (current_mode) {
    ...
    if (n > 0) {
        ...
        else if (strncmp(buf, "esc_start", 9) == 0) {
            // 测试负压软启动：从 0 到 30%，用时大约 2 秒
            // 假设控制环 (Update_Tick) 为 5ms，2秒 = 400个周期
            // 步长 = 30.0 / 400 = 0.075
            if (PID.is_running == 0) {
                PID.is_running = 1;           // 强制拉高运行标志，让主循环能够调用 esc_sys.Update_Tick()
                PID.base_target_speed = 0.0f; // 锁死底盘速度为 0，防止轮子乱转
            }
            esc_sys.Start_Soft(30.0f, 0.075f);
            printf("[VOFA] 测试负压软启动: 目标 30%%, 预计用时 2秒\n");
        }
        else if (strncmp(buf, "esc_stop", 8) == 0) {
            esc_sys.stop();
            PID.is_running = 0;
            motor_sys.Stop();
            printf("[VOFA] 测试结束, 负压与底盘停止\n");
        }
        // 外环模糊 PID 调参
        // else if (strncmp(buf, "bkp:", 4) == 0) KP_Base = atof(buf + 4);
        // else if (strncmp(buf, "bkd:", 4) == 0) KD_Base = atof(buf + 4);
        ...
        else if (strncmp(buf, "vrem:", 5) == 0) cfg_vofa_remote_enable = (bool)atof(buf + 5);
        else if (strncmp(buf, "vadj:", 5) == 0) vofa_target_speed_adjust = atof(buf + 5);
    }
}
