...
/********************************** 寄存器地址 *************************************/
#define LS_ATIM_BASE_ADDR   ( 0x16118000 )  // ATIM控制器基地址
...
/********************************* PWM 模式选择 ************************************/
#define LS_ATIM_Mode_1      ( 0x6 )         // PWM 模式 1
#define LS_ATIM_Mode_2      ( 0x7 )         // PWM 模式 2

/********************************* PWM 极性选择 ************************************/
#define LS_ATIM_NORMAL      ( 0x0 )         // 正相
#define LS_ATIM_INVERSED    ( 0x1 )         // 反相

/****************************** ATIM 控制器时钟频率 *********************************/
#define LS_ATIM_CLK         ( 160000000ul )

/********************************** 最大占空比 **************************************/
#define LS_ATIM_DUTY_MAX    ( 10000 )

class AtimPwm
{
public:
    AtimPwm();
    AtimPwm(uint8_t gpio, uint8_t chNum, uint8_t pol, uint32_t period, uint32_t duty_cycle, int mux = 0b11);
    void Enable(void);                      // 使能
...
public:
    uint8_t  Gpio;        // 引脚号
    uint8_t  ChNum;       // 通道号（0-3对应CH1-CH4）
    uint8_t  Pol;         // 极性
    uint8_t  Mode;        // 模式
    uint32_t Period;      // 周期
    uint32_t Duty_cycle;  // 占空比
private:
    void *ATIM_CCMR[2];   // 模式寄存器（0--输入；1--输出）
    void *ATIM_CCER;      // 使能寄存器
    void *ATIM_ARR;       // 自动重装载寄存器（周期）
    void *ATIM_CCRx;      // 捕获/比较寄存器1
    void *ATIM_CNT;       // 计数器寄存器
    void *ATIM_BDTR;      // 刹车和死区寄存器（ATIM特有）
};